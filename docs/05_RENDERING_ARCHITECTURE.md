...
Conventions: direct module imports, no barrels until a folder has a stable public surface (Sprint-1 decision); `draw/` modules export pure `(ctx, ...) => void` functions and may import only `theme.ts`, `assets.ts` and `src/shared` (correction recorded during R1: additionally, type-only imports from `src/lib/abyss-game` (`CreatureKind`) until the domain types move in the engine extraction, and `SHIP_IMPACT_FX_SECONDS` from `src/game/constants` until the R5 timing handoff removes it); `layers/` may additionally import `camera.ts` and `render-state.ts` (corrections recorded during R3: layer modules also import the geometry/presentation constants their moved code already referenced — `src/game/constants` (`SHIP_WORLD_Y`, `ANCHOR_SCREEN_Y_FRAC`, `ANCHOR_WORLD_OFFSET_PX`, `BG_*`, `SHIP_IMPACT_FX_SECONDS`) and `src/game/config` (`CHAIN_MAX_DEPTH`, `ANCHOR_DRAW_H`, `ANCHOR_RING_Y_FRAC`) — until the engine extraction relocates them, mirroring the R1/R2 convention; `effects.ts` imports the pure anchor sway/pin helpers (`anchorSwayX`, `anchorPinY`) exported by `layers/actor.ts`, so impact FX track the swaying anchor without duplicating the formula; `render-state.ts` declares the `RenderLayer` interface alongside the state/time types, with type-only imports of `Phase`/`Creature` from `src/lib/abyss-game` and `Camera` from `camera.ts`); only `scene-renderer.ts` is imported from outside `src/rendering` — it re-exports the contract types (`RenderState`, `RenderCreature`, `RenderTime`) so callers never reach into internals.
